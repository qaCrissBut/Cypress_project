describe("registration form testing", () => {
 ...
 it('testing the wrong data ', () => {
  ...
  cy.wait(1000);
  cy.get('div.hero-descriptor button.hero-descriptor_btn')
  .should('exist')
  .and('be.visible')
  .click();
  cy.get('.modal-content').should('be.visible');
  ...
  cy.get('input#signupEmail').type('duck@mal.com').blur();
  cy.get('input#signupEmail').should('have.css', 'color', 'rgb(73, 80, 87)');
  cy.get('div.invalid-feedback p').contains('Email is incorrect').should('be.visible');
...
//   cy.get('button[type="button"]').should('be.disabled');
  ...
  //   cy.get('input#signupRepeatPassword').type('11625F8a009');
  //   cy.wait(1000);
  //   cy.get('button[type="button"]').first().should('not.contain.disabled').click();
